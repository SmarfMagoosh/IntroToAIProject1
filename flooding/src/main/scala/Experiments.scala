 ...
 *
 * @param size   optionally restrict the size of the experiments (5, 15)
 * @param colors optionally restrict the number of colors for the experiments
 * @author Evan Dreher
 */
def experiment_suite(size: (Int, Int) = (5, 25), colors: (Int, Int) = (4, 8)): Unit = {
...
/** Performs all experiments using both greedy algorithms, runs much much quicker
 * so no restrictive options are necessary
 *
 * @author Evan Dreher
 */
def greedy_experiment_suite(): Unit = {
  // locate files to store results and add writers for them
  val files = List(
    new File("./samples/experiment_results/Greedy/max_greedy.csv"),
    new File("./samples/experiment_results/Greedy/min_greedy.csv"),
    new File("./samples/experiment_results/Greedy/improved_greedy.csv")
  )
  val writers = files.map(new PrintWriter(_))
  var ctr = 0
  for {
    ...
    println(s"Experiment #$ctr")
    val r = run_greedy_experiment(size, colors, id, algorithm)
    val pw = writers(algorithm)
    pw.write(s"$size, $colors, $id, N/A, ${r.runtime}, ${r.solution.mkString("[", " ", "]")}\n")
    pw.flush()
    ctr = ctr + 1
  }
  writers.foreach(_.close())
...
/** Runs a single experiment with one of the greedy algorithms
 *
 * @param size      the size of the experiment
 * @param colors    the number of colors involved in the experiment
 * @param id        the index
 ...
 * @author Evan Dreher
 */
def run_greedy_experiment(size: Int, colors: Int, id: Int, algorithm: Int): Result = {
  // load up the experiment from storage
  val g = get_experiment(size, colors, id)
  val algo = algorithm match
    case 0 => min_greedy
    case 1 => max_greedy
    case 2 => improved_greedy
  // measure time to get solution
  val s = System.currentTimeMillis()
  val solution = algo(g)
  Result(-1, System.currentTimeMillis() - s, solution.path)
}
